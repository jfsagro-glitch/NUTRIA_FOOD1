...
// реально смотрят. Именно точечные нули по ним выглядят как «пропала часть данных»
// (например: у йогурта заполнены другие витамины, а B12 = 0, хотя в молоке он есть).
// Группа при этом не пустая целиком, поэтому старая докомплектация (только по пустым
// группам) такие пробелы не закрывала. По этому набору решаем, что продукт стоит дозаполнить.
...
function hasMissingKeyMicronutrients(completeMicro: ReturnType<typeof buildCompleteMicronutrients>) {
  return Object.entries(KEY_MICRONUTRIENTS).some(([group, keys]) => {
    ...
    // распознан), иначе это просто «нет данных вообще» — покрывается другими проверками.
...
async function enrichProductMicronutrientsInBackground(product: { id: string; name: string; fiber?: number | null; micronutrients?: any }) {
  ...
  try {
    ...
    if (usdaMicro) {
      ...
      // значение (а не только целиком пустые группы) — так закрываются пробелы вроде
      // отсутствующего B12 у продукта, где остальные витамины уже заполнены. USDA —
      // точные данные, поэтому переписываем все нули, а не только «ключевые».
    ...
    // Что не покрыл USDA, добираем AI-оценкой (менее точно, но лучше нулей) — если
    // осталась пустая группа целиком ИЛИ пробел по ключевым нутриентам (напр. B12).
    if (hasEmptyMicronutrientGroup(merged as any) || hasMissingKeyMicronutrients(merged as any)) {
      ...
      if (estimatedMicro) {
        ...
        // Точечные нули по ключевым нутриентам добираем из AI-оценки (не заливаем все
        // нули подряд AI-догадками — только то, что пользователи реально смотрят).
  ...
  app.get("/api/products/barcode/:code", async (req, res) => {
    try {
      ...
      if (product) {
        ...
        // пустой таблицей витаминов/минералов. Раньше фоновая докомплектация запускалась
        // только в поиске, поэтому у отсканированных продуктов таблица так и оставалась
        // нулевой. Запускаем её и здесь (в фоне, не задерживая ответ): при следующем
        // открытии продукт уже будет с заполненными микроэлементами.
